...
def initialize_output(
    f1, h5f, bgroups=None, nineq=None, n2iwb=None, n3iwf=None, n3iwb=None, n4iwf=None, n4iwb=None, target=None, **kwargs
):
    for ineq in range(nineq):
...
def read_and_add(
    ...
    **kwargs
    ...
    if target == "1freq_b":
        x = h5in["{}/ineq-{:03}/p2iw-worm/{:05}/value".format(worm_group, ineq + 1, igr)][()] / float(
            prefactor * len(symgroups)
        )
        for gr in symgroups:
            if action == "+":
        ...
        x = h5in["{}/ineq-{:03}/p3iw-worm/{:05}/value".format(worm_group, ineq + 1, igr)][()] / float(
            prefactor * len(symgroups)
        )
        for iwb in range(2 * n3iwb + 1):
            for gr in symgroups:
        ...
        x = h5in["{}/ineq-{:03}/g4iw-worm/{:05}/value".format(worm_group, ineq + 1, igr)][()] / float(
            prefactor * len(symgroups)
        )
        ...
        for iwb in range(2 * n4iwb + 1):
            for gr in symgroups:
                if action == "+":
                    h5out["ineq-{:03}/{}/{:05}/{:05}/value".format(ineq + 1, channel, iwb, gr)][...] += x[
                        ..., iwb
                    ].transpose()
                elif action == "-":
                    h5out["ineq-{:03}/{}/{:05}/{:05}/value".format(ineq + 1, channel, iwb, gr)][...] -= x[
...
def main(conf):
    ...
    if conf["target"] == "1freq_f":  # we do this completely seperate since we only have to do sume numpy magic
        ...
        for ineq in range(conf["nineq"]):
            f2["dmft-last/ineq-{:03}/giw_unsymmetrized".format(ineq + 1)] = f2[
                "dmft-last/ineq-{:03}/giwk_obj".format(ineq + 1)
            ]
            del f2["dmft-last/ineq-{:03}/giwk_obj".format(ineq + 1)]
            f2["dmft-last/ineq-{:03}/giwk_obj/value".format(ineq + 1)] = np.zeros_like(
                f2["dmft-last/ineq-{:03}/giw_unsymmetrized/value".format(ineq + 1)], dtype=np.complex128
            )
            f2["dmft-last/ineq-{:03}/siw_unsymmetrized".format(ineq + 1)] = f2[
                "dmft-last/ineq-{:03}/siw".format(ineq + 1)
            ]
            del f2["dmft-last/ineq-{:03}/siw".format(ineq + 1)]
            f2["dmft-last/ineq-{:03}/siw/value".format(ineq + 1)] = np.zeros_like(
                f2["dmft-last/ineq-{:03}/siw_unsymmetrized/value".format(ineq + 1)], dtype=np.complex128
            )
            for band in range(conf["Nbands"][ineq]):
                for symband in conf["sym"][ineq][band]:
                    f2["dmft-last/ineq-{:03}/giwk_obj/value".format(ineq + 1)][band, :, :] += np.mean(
                        f2["dmft-last/ineq-{:03}/giw_unsymmetrized/value".format(ineq + 1)][symband - 1, :, :]
                        / float(len(conf["sym"][ineq][band])),
                        axis=0,
                    )
                    f2["dmft-last/ineq-{:03}/siw/value".format(ineq + 1)][band, :, :] += np.mean(
                        f2["dmft-last/ineq-{:03}/siw_unsymmetrized/value".format(ineq + 1)][symband - 1, :, :]
                        / float(len(conf["sym"][ineq][band])),
                        axis=0,
                    )
        f2.close()
        ...
        for ineq in range(conf["nineq"]):
            for ch in ["dens", "magn"]:
                for gr in conf["groups"][ineq]:
                    ...
                    print(
                        "group {},".format(gr["group"]),
                        "channel: {},".format(ch),
                        "action: {},".format(action),
                        "{} equivaluent band groups:".format(len(symgroups)),
                        symgroups,
                    )
