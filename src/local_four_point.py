...
class LocalFourPoint(LocalNPoint, IHaveChannel):
    ...
    def __add__(self, other) -> "LocalFourPoint":
        ...
        """
        return self._execute_add(other)
    ...
    @staticmethod
    def identity(n_bands: int, niw: int, niv: int) -> "LocalFourPoint":
        mat = np.tile(
            np.einsum("ad,bc,vp->abcdvp", np.eye(n_bands), np.eye(n_bands), np.eye(2 * niv))[..., None, :, :],
            (1, 1, 1, 1, 2 * niw + 1, 1, 1),
        )
        return LocalFourPoint(mat)
    ...
    def _execute_matmul(self, other, left_hand_side: bool = True) -> "LocalFourPoint":
        """
