...
LABELS = {'Gamma': r'$\Gamma$', 'X': 'X', 'Y': 'Y', 'M': 'M', 'M2': 'M/2', 'Z': 'Z', 'R': 'R', 'A': 'A'}
...
def shift_mat_by_pi(mat, nk=None):
    if nk is None:
        nk = [mat.current_shape[0], mat.current_shape[1]]
...
def find_zeros(mat):
    ...
    '''
    ind_x = np.arange(mat.current_shape[0])
    ind_y = np.arange(mat.current_shape[1])
...
def x_y_sym(mat):
    ...
    assert len(np.shape(mat)) >= 3, f'dim(mat) = {len(np.shape(mat))} but must be at least 3 dimensional'
    if mat.current_shape[0] == mat.current_shape[1]:
...
class KGrid:
    ...
    def get_k_slice(self, mat, k_slice_dim=0, k_slice_val=0):
        ...
        assert mat.current_shape[:3] == self.nk, 'mat has to be of shape [nkx,nky,nkz,...]'
